...
class ScaleFinder:
    """
    Identifies a scale given a key and it's relevant mode. Uses two tuples for keys and modes as well as a dictionary
    to expand on the intervals for each given mode. Key select defines the starting point in the tuple and mode select
    defines the intervals. GUI build in tkinter.
    """
    def __init__(self):
        """
        Creates the tuples for keys and modes and the dictionary for mode intervals.
        Defines tkinter window and style as well as populates it with labels, buttons and combo-boxes.
        """
        # ----------Variables---------- #
    ...
    def get_scale(self):
        """
        Gets selected key and mode from combo-boxes. Uses key to set starting point and mode to define intervals
        which are compare to tuple of keys to create list of new notes to append to list scale.
        """
        # ----------ComboBox Variables---------- #
